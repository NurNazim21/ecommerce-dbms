...
if ($_SESSION['role'] !== 'category_manager') {
    header("Location: ../auth/login.php");
    exit();
}

$cm_id = intval($_SESSION['user_id']);
...
$extra  = $filter === 'unread' ? " AND is_read = 0" : ($filter === 'read' ? " AND is_read = 1" : "");
...
$type_icons = [
    'new_product_pending'  => ['📦', '#fee2e2', '#991b1b'],
    'product_approved'     => ['✅', '#d1fae5', '#065f46'],
    'product_rejected'     => ['❌', '#fee2e2', '#991b1b'],
    'new_category_request' => ['🏷', '#fef9c3', '#854d0e'],
    'category_approved'    => ['✅', '#d1fae5', '#065f46'],
    'category_rejected'    => ['❌', '#fee2e2', '#991b1b'],
];
?>
...
.ftab.active{background:#6d28d9;color:#fff;border-color:#6d28d9;}
...
.notif-item{display:flex;align-items:flex-start;gap:.9rem;background:#fff;border-radius:12px;padding:1rem 1.1rem;box-shadow:0 1px 3px rgba(0,0,0,.07);border:.5px solid #e5e7eb;text-decoration:none;color:inherit;transition:box-shadow .13s;position:relative;}
.notif-item:hover{box-shadow:0 3px 12px rgba(0,0,0,.1);}
.notif-item.unread{border-left:3px solid #6d28d9;background:#faf5ff;}
...
.unread-dot{width:8px;height:8px;border-radius:50%;background:#6d28d9;position:absolute;top:12px;right:12px;}
.btn-mark-all{background:#6d28d9;color:#fff;border:none;border-radius:8px;padding:7px 16px;font-size:.82rem;font-weight:600;cursor:pointer;}
.btn-mark-all:hover{background:#5b21b6;}
...
<div class="container" style="max-width:720px;">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;gap:10px;align-items:center;">
            <a href="dashboard.php" style="font-size:.82rem;color:#6b7280;text-decoration:none;">← Dashboard</a>
            <?php if ($unread_count > 0): ?>
            <form method="POST">
    ...
    <div class="filter-tabs">
        ...
        <a href="notifications.php?filter=read"   class="ftab <?= $filter==='read'   ? 'active':'' ?>">Read</a>
    </div>
    ...
    <div class="notif-list">
        <?php foreach ($notifications as $n):
            ...
            $href = 'notifications.php?read=' . intval($n['id']) .
                    ($n['link'] ? '&link=' . urlencode($n['link']) : '');
        ?>
    ...
    <div class="empty-state">
        ...
        <p style="font-size:.83rem">New product submissions and category requests will appear here.</p>
    </div>
    <?php endif; ?>
