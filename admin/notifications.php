...
if (isset($_GET['read']) && intval($_GET['read']) > 0) {
    ...
    $stmt->bind_param("ii", $nid, $admin_id);
    $stmt->execute();
    $stmt->close();
    if ($redir) {
...
$stmt = $conn->prepare("SELECT * FROM notifications WHERE user_id = ? $extra ORDER BY created_at DESC LIMIT 80");
$stmt->bind_param("i", $admin_id);
$stmt->execute();
...
$type_icons = [
    ...
    'payout_update'       => ['💰', '#fef9c3', '#854d0e'],
     // ── admin-specific ───────────────────────────────────────────────
    'new_order'            => ['🛒', '#d1fae5', '#065f46'],
    'new_seller_request'   => ['🏪', '#fef9c3', '#854d0e'],
    'new_cm_request'       => ['🗂', '#f3e8ff', '#6d28d9'],
...
.ftab.active{background:#0f3460;color:#fff;border-color:#0f3460;}
...
.notif-item.unread{border-left:3px solid #0f3460;background:#f8faff;}
...
.unread-dot{width:8px;height:8px;border-radius:50%;background:#0f3460;position:absolute;top:12px;right:12px;}
.btn-mark-all{background:#0f3460;color:#fff;border:none;border-radius:8px;padding:7px 16px;font-size:.82rem;font-weight:600;cursor:pointer;}
.btn-mark-all:hover{background:#1a5276;}
...
<div class="container" style="max-width:720px;">
    ...
    <div class="empty-state">
        ...
        <p style="font-size:.83rem">Refund requests, order events and seller alerts will appear here.</p>
    </div>
    <?php endif; ?>
