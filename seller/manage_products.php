...
// === BLOCK NON-APPROVED SELLERS ===
if ($user['seller_status'] !== 'approved') {
    include("../includes/header.php");
    ?>
    <div class="container py-5">
        <div class="alert alert-warning text-center shadow">
            <h4>⏳ Your seller account is not approved yet.</h4>
            <p>Please wait for admin approval before adding products.</p>
...
if (isset($_POST['save_product'])) {
    ...
    // Default Image
    $image_path = $_POST['old_image'] ?? 'assets/images/no-image.png';
    ...
    if (!isset($error)) {
        ...
        if ($id > 0) {
            ...
            $sql = "INSERT INTO products 
                    (name, description, price, stock, brand, category_id, image, seller_id, status)
                    VALUES
                    ('$name', '$description', $price, $stock, '$brand', $category_id, '$image_path', $user_id, 'pending')";

            $success_message = "Product submitted for approval successfully!";
        }
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM products WHERE id = $delete_id AND seller_id = $user_id");
...
if (isset($_GET['edit'])) {
    ...
    $edit_product = $conn->query("SELECT * FROM products WHERE id = $edit_id AND seller_id = $user_id")->fetch_assoc();
    if ($edit_product) {
        $edit_mode = true;
    }
}
...
$where_clauses = ["p.seller_id = $user_id"];
...
$count_result = $conn->query("SELECT status, COUNT(*) as cnt FROM products WHERE seller_id = $user_id GROUP BY status");
...
.btn-submit {
    background: linear-gradient(90deg, #1a7a4a, #28a745);
    color: #fff;
    border: none;
    border-radius: 8px;
    padding: 10px 32px;
...
<div class="container">
    ...
    <div class="page-header-card">
        <div>
            <h1>🛒 <?= $edit_mode ? 'Edit Product' : 'My Products' ?></h1>
            <div class="subtitle">Manage your product listings</div>
        </div>
    ...
    <!-- ===== ADD / EDIT FORM ===== -->
    <div class="form-card">
        <div class="card-header <?= $edit_mode ? 'bg-warning text-dark' : 'bg-success text-white' ?>">
            ...
            <form method="POST" enctype="multipart/form-data" class="row g-3">
                ...
                <div class="col-12 d-flex gap-2">
                    <button type="submit" name="save_product" class="btn-submit btn">
                        <?= $edit_mode ? 'Update Product' : 'Submit Product' ?>
                    </button>
                    <?php if ($edit_mode): ?>
