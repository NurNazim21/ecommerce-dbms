...
if (isset($_GET['read']) && intval($_GET['read']) > 0) {
    ...
    $stmt->bind_param("ii", $nid, $seller_id);
    $stmt->execute();
    $stmt->close();
    if ($redir) {
...
.ftab.active{background:#2d6a4f;color:#fff;border-color:#2d6a4f;}
...
.notif-item.unread{border-left:3px solid #2d6a4f;background:#f7faf8;}
...
.unread-dot{width:8px;height:8px;border-radius:50%;background:#2d6a4f;position:absolute;top:12px;right:12px;}
.btn-mark-all{background:#2d6a4f;color:#fff;border:none;border-radius:8px;padding:7px 16px;font-size:.82rem;font-weight:600;cursor:pointer;}
...
<div class="container" style="max-width:720px;">
    ...
    <div class="empty-state">
        ...
        <p style="font-size:.83rem">New orders, status updates and payout confirmations will appear here.</p>
    </div>
    <?php endif; ?>
